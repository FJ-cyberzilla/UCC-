"""
Quora username checker with question-based detection
"""
...
class QuoraChecker(BasePlatform):
    """Quora username availability checker"""
    ...
    async def check_username(self, username: str) -> Dict[str, Any]:
        """Check Quora username availability"""
    ...
    async def analyze_response(self, response, username: str) -> Dict[str, Any]:
        """Analyze Quora response"""
        
        if response.status == 200:
            content = await response.text()
            
            # Quora profile indicators
            if 'quora.com/profile/' in content and 'profile' in content:
