"""
Bluesky (bsky.app) username checker
"""
...
class BlueskyChecker(BasePlatform):
    ...
    async def analyze_response(self, response, username: str) -> Dict[str, Any]:
        """Analyze Bluesky response"""
        
        if response.status == 200:
            content = await response.text()
            
            # Bluesky profile indicators
            if 'bsky.app/profile/' in content and 'profile' in content:
