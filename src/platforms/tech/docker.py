...
class DockerChecker(BasePlatform):
    ...
    async def analyze_response(self, response, username: str) -> Dict[str, Any]:
        """Analyze Docker Hub response"""
        
        if response.status == 200:
            content = await response.text()
            
            # Docker Hub user indicators
            if 'hub.docker.com/u/' in content and 'profile' in content:
