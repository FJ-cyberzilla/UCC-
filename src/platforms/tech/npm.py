...
class NpmChecker(BasePlatform):
    """npm username availability checker"""
    
    def __init__(self, session: aiohttp.ClientSession, anti_detection=None):
        super().__init__(session, anti_detection)
        self.platform_name = "npm"
        self.base_url = "https://npmjs.com"
    ...
    async def analyze_response(self, response, username: str) -> Dict[str, Any]:
        """Analyze npm response"""
        
        if response.status == 200:
            content = await response.text()
            
            # npm user indicators
            if 'npmjs.com/~' + username in content and 'profile' in content:
                return self.create_result(
                    exists=True,
                    confidence=0.95,
                    url=f"https://npmjs.com/~{username}",
                    method="http"
                )
            else:
                return self.create_result(
